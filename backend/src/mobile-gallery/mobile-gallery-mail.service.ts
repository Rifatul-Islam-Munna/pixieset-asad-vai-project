...
export class MobileGalleryMailService {
  ...
  async sendInvite(userId: string, appId: string, body: Record<string, any>) {
    ...
    const from = (
      this.configService.get<string>('MOBILE_GALLERY_EMAIL_FROM') ||
    ...
    if (!apiKey || !from) {
      return {
        delivered: false,
        provider: 'mailto',
        reason: 'Email delivery is not configured. The frontend can open the prepared invite in the user email app.',
      };
    }
    ...
    const replyTo = this.isEmail(String(profile?.contactEmail || '').trim())
      ...
      : undefined;
    const sendCopy = Boolean(body.sendCopy && replyTo && replyTo.toLowerCase() !== to);
    ...
    if (!response?.ok) {
      ...
      throw new BadGatewayException(details || 'Email provider could not send the invitation');
    }
...
<html>
  <body style="margin:0;background:#f5f5f4;font-family:Arial,sans-serif;color:#222;">
    <div style="max-width:620px;margin:0 auto;padding:32px 16px;">
      ...
      ${input.businessName ? `<p style="text-align:center;color:#999;font-size:12px;margin:18px 0 0;">${this.escapeHtml(input.businessName)}</p>` : ''}
    </div>
  </body>
</html>`;
  }
