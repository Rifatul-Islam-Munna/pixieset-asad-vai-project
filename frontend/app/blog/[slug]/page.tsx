    ...
    <main className="min-h-screen bg-white text-[#111]">
      ...
      <article>
        <header className="border-y border-[#eee9fb] bg-[#fbfaff] px-5 py-16 sm:py-24">
          <div className="mx-auto max-w-4xl"><Link href="/blog" className="inline-flex items-center gap-2 text-sm font-bold text-[#6337d8]"><ArrowLeft className="size-4" />Back to Blog</Link><p className="mt-8 text-xs font-bold uppercase tracking-[.18em] text-[#6337d8]">{post.featured ? "Featured · " : ""}{post.category || "Guides"} · {post.language || "English"}</p><h1 className="mt-4 text-4xl font-bold leading-[1.08] tracking-[-.045em] sm:text-6xl">{post.title}</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-[#666]">{post.excerpt}</p><div className="mt-8 flex flex-wrap gap-5 text-sm text-[#777]"><span className="inline-flex items-center gap-2"><CalendarDays className="size-4 text-[#6337d8]" />{new Date(post.publishedAt || post.createdAt || "1970-01-01").toLocaleDateString()}</span>{post.author && <span className="inline-flex items-center gap-2"><User className="size-4 text-[#6337d8]" />{post.author}</span>}</div></div>
        </header>
