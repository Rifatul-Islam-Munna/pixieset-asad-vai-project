...
export function PhotographerTypesShowcase({ section }: Props) {
  ...
  const smooth = useSpring(scrollYProgress, { stiffness: 68, damping: 23, mass: 0.34 });
  const imageY = useTransform(smooth, [0, 1], [32, -34]);
  const imageRotate = useTransform(smooth, [0, 1], [-1.5, 1.5]);
  const sideY = useTransform(smooth, [0, 1], [26, -22]);
  ...
  return (
    <section ref={rootRef} data-home-motion-managed className="relative bg-white" style={{ minHeight: reduceMotion ? undefined : `${Math.max(2, tabs.length) * 95}vh` }}>
      <div className={reduceMotion ? "px-4 py-16 sm:px-5 md:px-8" : "sticky top-0 flex min-h-screen items-center overflow-hidden px-4 py-10 sm:px-5 md:px-8"}>
        ...
        <div className="relative mx-auto w-full max-w-[1180px]">
          ...
          <div className="mt-10 grid items-center gap-8 lg:grid-cols-[minmax(0,1fr)_290px] lg:gap-14">
            <motion.div className="relative overflow-hidden rounded-[22px] bg-[#f6f1ff] shadow-[0_32px_90px_rgba(54,35,95,.14)]" style={reduceMotion ? undefined : { y: imageY, rotate: imageRotate }}>
            ...
            <motion.div className="relative flex flex-col justify-center gap-1" style={reduceMotion ? undefined : { y: sideY }}>
              {tabs.map((tab, index) => {
                const selected = index === activeIndex;
                return <motion.button key={tab.value} type="button" onClick={() => goTo(index)} whileHover={reduceMotion ? undefined : { x: 8 }} whileTap={reduceMotion ? undefined : { scale: .98 }} className={`group relative w-full overflow-hidden px-0 py-3 text-left text-lg font-semibold transition sm:text-xl ${selected ? "text-[#6337d8]" : "text-[#b2acba] hover:text-[#7a63c9]"}`}>
