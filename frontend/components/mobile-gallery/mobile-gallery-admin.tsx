      ...
      <div className="border-b bg-[#f7f7f7] px-4 sm:px-8">
        <div className="mx-auto flex h-14 max-w-[1180px] items-center justify-between">
          <div className="relative">
            <button onClick={() => setSwitcherOpen((value) => !value)} className="flex items-center gap-3 font-semibold">
            ...
            {switcherOpen && (
              <div className="absolute left-0 top-11 z-50 w-[320px] border bg-white p-3 shadow-xl">
      ...
      <form onSubmit={save} className="mx-auto max-w-[920px] px-4 py-10 sm:px-8">
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2">
          <label className="sm:col-span-2 text-sm font-semibold">Logo
            <div className="mt-2 flex items-center gap-5 border p-4">
      ...
      <section className="mx-auto max-w-[1100px] px-4 py-8 sm:px-8">
        <div className="flex flex-wrap items-center justify-between gap-5">
          <div className="flex items-center gap-4">
            <Link href="/dashboard/mobile-gallery"><ArrowLeft className="size-5" /></Link>
            {app.iconUrl || app.coverImage ? <img src={app.iconUrl || app.coverImage} alt="" className="size-16 rounded-2xl object-cover" /> : <div className="flex size-16 items-center justify-center rounded-2xl bg-[#eee]"><Smartphone className="size-6" /></div>}
            <div><h1 className="text-2xl font-medium">{app.name}</h1><p className="mt-1 text-xs uppercase tracking-wider text-[#888]">{app.status}</p></div>
          </div>
    ...
    <section className="grid gap-8 pt-7 lg:grid-cols-[1fr_430px]">
      <div>
        <Panel title="Cover Style">
          ...
          <div className="mt-5 grid grid-cols-2 gap-4"><label className="text-xs font-semibold">Focal X<input type="range" min="0" max="100" value={design.focal?.x ?? 50} onChange={(event) => patchDesign({ focal: { x: Number(event.target.value), y: design.focal?.y ?? 50 } })} className="mt-2 w-full" /></label><label className="text-xs font-semibold">Focal Y<input type="range" min="0" max="100" value={design.focal?.y ?? 50} onChange={(event) => patchDesign({ focal: { x: design.focal?.x ?? 50, y: Number(event.target.value) } })} className="mt-2 w-full" /></label></div>
        </Panel>
        ...
        <Panel title="App Icon"><div className="flex items-center gap-4">{draft.iconUrl ? <img src={draft.iconUrl} alt="" className="size-20 rounded-2xl object-cover" /> : <div className="flex size-20 items-center justify-center rounded-2xl bg-[#eee]"><Smartphone className="size-6" /></div>}<label className="relative cursor-pointer border px-4 py-2 text-sm">Upload Icon<input type="file" accept="image/*" className="absolute inset-0 cursor-pointer opacity-0" onChange={(event) => uploadIcon(event.target.files?.[0])} /></label></div></Panel>
...
function AppSettingsEditor({ app, updateApp }: { app: MobileGalleryApp; updateApp: any }) {
  ...
  return (
    <form onSubmit={save} className="max-w-[760px] space-y-7 pt-7">
      ...
      <label className="block text-sm font-semibold">Event Date<input type="date" value={form.eventDate ? new Date(form.eventDate).toISOString().slice(0, 10) : ""} onChange={(event) => setForm((current) => ({ ...current, eventDate: event.target.value }))} className="mt-2 h-12 w-full border px-4 font-normal outline-none" /></label>
...
function ShareScreen({ app }: { app: MobileGalleryApp }) {
  ...
  return (
    <main className="min-h-screen bg-[#f5f5f4] text-[#202020]">
      <div className="border-b bg-white px-5 py-4"><Link href={`/dashboard/mobile-gallery/apps/${app._id}`} className="flex items-center gap-2 text-sm"><ArrowLeft className="size-4" /> Back to {app.name}</Link></div>
      <div className="grid min-h-[calc(100vh-58px)] lg:grid-cols-2">
        <form onSubmit={send} className="bg-white p-6 sm:p-12">
          <h1 className="text-2xl font-light">Share by Email</h1>
          <Field label="Email" value={email} onChange={setEmail} type="email" required placeholder="e.g. johnsmith@email.com" />
          <label className="mt-5 block text-sm font-semibold">Template<select value={template} onChange={(event) => setTemplate(event.target.value)} className="mt-2 h-12 w-full border px-4 font-normal"><option>Your Mobile Gallery App is Ready</option><option>Wedding Gallery Delivery</option><option>Event Photos Are Ready</option></select></label>
          <Field label="Subject" value={subject} onChange={setSubject} />
          <textarea readOnly value={body} rows={8} className="mt-5 w-full border p-4 text-sm leading-6 text-[#555]" />
          <div className="mt-5 flex flex-wrap gap-3"><button className="flex items-center gap-2 bg-[#18bfa6] px-6 py-3 font-semibold text-white"><Mail className="size-4" /> Send Invite</button><button type="button" onClick={() => navigator.clipboard.writeText(link).then(() => toast.success("Link copied"))} className="flex items-center gap-2 border px-5 py-3"><Copy className="size-4" /> Copy Link</button><a href={link} target="_blank" rel="noreferrer" className="flex items-center gap-2 border px-5 py-3"><ExternalLink className="size-4" /> Open Link</a></div>
        </form>
        <div className="flex items-center justify-center p-8"><div className="w-full max-w-xl bg-white shadow"><div className="border-b p-10 text-center"><p className="text-xs uppercase tracking-widest text-[#777]">{template}</p><h2 className="mt-6 text-3xl">Your Mobile Gallery App is Ready</h2></div><div className="p-10 text-center">{app.iconUrl || app.coverImage ? <img src={app.iconUrl || app.coverImage} alt="" className="mx-auto size-36 rounded-[30px] object-cover" /> : null}<h3 className="mt-4 uppercase tracking-widest">{app.name}</h3><a href={link} className="mt-8 inline-block bg-[#18bfa6] px-7 py-3 font-semibold text-white">Install App</a></div></div></div>
      </div>
    </main>
  );
}
...
function PhonePreview({ children }: { children: ReactNode }) {
  return <div className="mx-auto w-full max-w-[390px] rounded-[42px] bg-white p-3 shadow-[0_22px_60px_rgba(0,0,0,0.16)]"><div className="h-[720px] overflow-hidden rounded-[32px] border bg-white">{children}</div></div>;
}
