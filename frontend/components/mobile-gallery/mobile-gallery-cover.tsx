...
export function MobileGalleryCover({
  ...
  const text = resolveCoverText(app, themeName, design.coverText);

  if (coverStyle === "full" && cover) {
    const vertical = text.verticalPosition === "top"
      ? "items-start pt-12"
      : text.verticalPosition === "center"
        ? "items-center"
        : "items-end pb-12";
    return (
      <header className="relative h-[56vh] min-h-[360px] overflow-hidden">
        ...
        <div className={`absolute inset-0 flex px-6 ${vertical}`}>
          <CoverCopy app={app} theme={themeName} text={text} inverse />
        </div>
      </header>
    );
  }
    ...
    return (
      <header>
        <div className="px-6 pb-8 pt-12">
          <CoverCopy app={app} theme={themeName} text={text} />
        </div>
...
function CoverCopy({
  ...
  const color = inverse ? text.textColor || "#ffffff" : undefined;
  ...
  const titleStyle: CSSProperties = {
    fontSize: `clamp(${Math.max(22, text.titleSize * 0.72)}px, 8vw, ${text.titleSize}px)`,
...
function resolveCoverText(app: MobileGalleryApp, theme: ThemeName, custom?: MobileGalleryCoverText): ResolvedCoverText {
  const defaults: Record<ThemeName, ResolvedCoverText> = {
    echo: {
      eyebrow: "Mobile Gallery",
      title: app.name,
      subtitle: "",
      showDate: true,
      dateLabel: "",
    ...
    spring: {
      eyebrow: "A collection of moments",
      title: app.name,
      subtitle: "",
      showDate: true,
      dateLabel: "",
    ...
    sage: {
      eyebrow: "Private mobile gallery",
      title: app.name,
      subtitle: "",
      showDate: true,
      dateLabel: "",
  ...
  return { ...defaults[theme], ...(custom || {}), title: custom?.title || app.name };
}
